@extends('layouts.app')
@section('content')
<div class="d-flex justify-content-between align-items-center mb-3"><div><h4 class="mb-1">Penjualan Baru</h4><div class="text-secondary small">Buat transaksi penjualan</div></div><a href="{{ route('inventori.penjualan') }}" class="btn btn-outline-secondary">← Kembali ke Penjualan</a></div>
<div class="card shadow-sm"><div class="card-body">
<div class="row g-3"><div class="col-md-6"><label class="form-label">Tanggal</label><input type="date" class="form-control" value="{{ now()->toDateString() }}" readonly></div><div class="col-md-6"><label class="form-label">Nomor</label><input class="form-control" value="Otomatis" readonly></div><div class="col-md-6"><label class="form-label">Cari Customer</label><div class="input-group"><input id="customerSearch" class="form-control" placeholder="Cari customer..." readonly><button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#customerModal">🔍</button></div></div><div class="col-md-6"><label class="form-label">Pilih Unit</label><select id="unitSelect" class="form-select" required><option value="">Pilih Unit</option>@foreach($units as $u)<option value="{{ $u->id }}">{{ $u->name }}</option>@endforeach</select></div></div>
...
<div class="modal fade" id="itemModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-sm modal-dialog-centered"><div class="modal-content"><div class="modal-header py-2"><h6 class="modal-title">Cari Item</h6><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body p-2"><input id="itemFilter" class="form-control form-control-sm mb-2" placeholder="Ketik nama / kode..."><div class="list-group list-group-flush">@foreach($products as $p)<button type="button" class="list-group-item list-group-item-action item-choice" data-id="{{ $p->id }}" data-name="{{ $p->name }}" data-code="{{ $p->code ?? $p->sku }}" data-unit="{{ $p->selling_unit_code ?? '-' }}" data-price="{{ $p->selling_price ?? 0 }}">{{ $p->code ?? $p->sku }} — {{ $p->name }}</button>@endforeach</div></div></div></div></div>
@endsection
...
(function () {
    ...
    function calculateTotals() {
        const subtotal = selectedItem
            ? (Number(selectedItem.qty) * Number(selectedItem.unit_price)) - Number(selectedItem.discount || 0)
            : 0;
    ...
    document.querySelectorAll('.customer-choice').forEach(function (button) {
    ...
    document.getElementById('customerFilter').addEventListener('input', function (e) {
        ...
        document.querySelectorAll('.customer-choice').forEach(function (button) {
            button.classList.toggle('d-none', !button.textContent.toLowerCase().includes(q));
        });
    });
    ...
    document.querySelectorAll('.item-choice').forEach(function (button) {
        button.addEventListener('click', function () {
            ...
            row.innerHTML = '<td><b>' + button.dataset.code + '</b><div class="small text-secondary">' + button.dataset.name + '</div></td>' +
                '<td>' + button.dataset.unit + '</td>' +
                '<td class="text-end">1</td>' +
                '<td class="text-end">' + rupiah(button.dataset.price) + '</td>' +
                '<td class="text-end">' + rupiah(button.dataset.price) + '</td>' +
                '<td class="text-end"><button type="button" class="btn btn-sm btn-outline-danger remove-item">×</button></td>';
    ...
    document.getElementById('paymentMethod').addEventListener('change', function (e) {
        document.getElementById('dueDate').classList.toggle('d-none', e.target.value !== 'Kredit / Bon');
    });

    document.getElementById('saveSales').addEventListener('click', async function () {
        try {
            if (!document.getElementById('unitSelect').value) throw new Error('Unit wajib dipilih.');
            ...
            const payload = {
                ...
                memo: document.querySelector('textarea').value || null,
