                ...
                <tr>
                    ...
                    <td class="text-end text-nowrap">
                        <button type="button" class="btn btn-outline-primary btn-sm btn-edit-master"
                            data-id="{{ $row->id }}"
                            data-row='{{ json_encode($row, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_AMP|JSON_HEX_QUOT) }}'>Edit</button>
                    ...
                    @foreach($config['fields'] as $key=>$field)
                        <div class="col-md-4">
                            ...
                            @if($field['type']==='textarea')
                                <textarea name="{{ $key }}" class="form-control" rows="2" @if($field['required']??false) data-required="1" @endif></textarea>
                            @elseif($field['type']==='select')
                                <select name="{{ $key }}" class="form-select" @if($field['required']??false) data-required="1" @endif>
                                    @foreach($field['options'] as $value=>$label)<option value="{{ $value }}">{{ $label }}</option>@endforeach
                                </select>
                            @else
                                <input name="{{ $key }}" type="{{ $field['type'] }}" step="{{ $field['step'] ?? 'any' }}" class="form-control" @if($field['required']??false) data-required="1" @endif>
                            @endif
                        </div>
                    @endforeach
                    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (missing.length) {
            showMessage('Field wajib diisi:<br><strong>' + missing.join('<br>') + '</strong>', 'danger');
            return;
        }
