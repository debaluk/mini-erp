                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex gap-4">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="is_active" value="1" id="supplier-active" checked>
                            <label class="form-check-label" for="supplier-active">Aktif</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="is_active" value="0" id="supplier-inactive">
                            <label class="form-check-label" for="supplier-inactive">Nonaktif</label>
                        </div>
                    </div>
                </div>
...
<div class="card shadow-sm">
    <div class="card-body py-2 border-bottom">
        <div class="fw-semibold">Daftar Supplier</div>
    </div>
...
(function () {
    ...
    function alertBox(message, kind = 'success') {
        ...
        if (!box) return;
        box.innerHTML = '<div class="alert alert-' + kind + ' py-2 small mb-0">' + message + '</div>';
    ...
    function resetForm() {
        ...
        el('supplier-name').classList.remove('is-invalid');
    }
    ...
    window.submitSupplierForm = async function (event) {
        ...
        if (!name.value.trim()) {
            ...
            alertBox('Nama Supplier wajib diisi.', 'danger');
            name.focus();
            return false;
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                alertBox(data.message || errors.join('<br>') || 'Gagal menyimpan supplier.', 'danger');
                return false;
            }
            ...
            console.error(error);
            alertBox('Terjadi kesalahan saat menyimpan supplier.', 'danger');
        } finally {
    ...
    document.addEventListener('DOMContentLoaded', function () {
            ...
            table = new DataTable('#suppliers-table', {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: row =>
                            '<button type="button" class="btn btn-outline-primary btn-sm btn-edit-supplier" data-id="' + row.id + '">Edit</button> ' +
                            '<button type="button" class="btn btn-outline-danger btn-sm btn-delete-supplier" data-id="' + row.id + '">Hapus</button>'
                    }
                ]
            });
        }
        ...
        document.addEventListener('click', function (event) {
            ...
            if (edit && table) {
                ...
                el('supplier-code').value = row.code || '';
                el('supplier-name').value = row.name || '';
                el('supplier-phone').value = row.phone || '';
                el('supplier-address').value = row.address || '';
                el('supplier-active').checked = Number(row.is_active) === 1;
                el('supplier-inactive').checked = Number(row.is_active) !== 1;
                el('supplier-submit').textContent = 'Update';
                el('supplier-cancel').classList.remove('d-none');
                el('supplier-name').focus();
                window.scrollTo({ top: 0, behavior: 'smooth' });
                return;
            }

            const del = event.target.closest('.btn-delete-supplier');
            if (!del) return;

            if (!confirm('Hapus supplier ini? Jika sudah digunakan transaksi, supplier tidak dapat dihapus. Nonaktifkan jika masih diperlukan untuk histori.')) {
                return;
            }

            del.disabled = true;

            fetch(@json(url('/master/supplier')) + '/' + del.dataset.id, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest',
                ...
                if (!response.ok) {
                    alertBox(data.message || 'Supplier tidak dapat dihapus.', 'danger');
                    return;
                }

                alertBox(data.message || 'Supplier berhasil dihapus.');
                if (editId === del.dataset.id) resetForm();
                table?.ajax.reload(null, false);
            }).catch(error => {
                console.error(error);
                alertBox('Terjadi kesalahan saat menghapus supplier.', 'danger');
            }).finally(() => {
                del.disabled = false;
            });
        });
