...
<div class="col-lg-3"><label class="form-label">Supplier</label><select name="supplier_id" class="form-select" required><option value="">Pilih supplier</option>@foreach($suppliers as $s)<option value="{{ $s->id }}">{{ $s->code }} — {{ $s->name }}</option>@endforeach</select></div><div class="col-lg-3"><label class="form-label">Produk</label><select name="product_id" id="purchaseProduct" class="form-select" required><option value="">Pilih produk</option>@foreach($products as $p)<option value="{{ $p->id }}" data-base-unit-id="{{ $p->base_unit_id }}">{{ $p->name }}</option>@endforeach</select></div><div class="col-lg-2"><label class="form-label">Satuan</label><select name="unit_id" id="purchaseUnit" class="form-select" required></select></div><div class="col-lg-3"><label class="form-label">Gudang</label><select name="warehouse_id" class="form-select" required>@foreach($warehouses as $w)<option value="{{ $w->id }}">{{ $w->name }}</option>@endforeach</select></div><div class="col-sm-6 col-lg-2"><label class="form-label">Qty</label><input name="qty" type="number" step="0.001" min="0.001" class="form-control" required></div><div class="col-sm-6 col-lg-2"><label class="form-label">Harga Beli / Satuan</label><input name="unit_cost" type="number" step="0.01" min="0" class="form-control" required></div><div class="col-lg-1 d-flex align-items-end"><button class="btn btn-primary w-100">Simpan</button></div></form><script>
(function(){
 const products=@json($products->map(fn($p)=>['id'=>$p->id,'base_unit_id'=>$p->base_unit_id,'base_unit_name'=>$p->base_unit_name])->values());
 const conversions=@json($productUoms->map(fn($rows,$pid)=>[$pid=>$rows->values()])->values());
 const map={}; conversions.forEach(x=>Object.assign(map,x));
 const product=document.getElementById('purchaseProduct'), unit=document.getElementById('purchaseUnit');
 function render(){ const p=products.find(x=>Number(x.id)===Number(product.value)); unit.innerHTML=''; if(!p)return;
   const base=document.createElement('option'); base.value=p.base_unit_id; base.textContent=(p.base_unit_name||'Base Unit')+' — Base'; base.dataset.factor='1'; unit.appendChild(base);
   (map[p.id]||[]).forEach(x=>{const o=document.createElement('option');o.value=x.unit_id;o.textContent=x.code+' — 1 = '+x.conversion_factor+' Base';o.dataset.factor=x.conversion_factor;if(Number(x.is_default_purchase))o.selected=true;unit.appendChild(o);});
 }
 product.addEventListener('change',render); render();
})();
</script></div></div>
@endif
@if($module === 'movements')
...
@php
    $groups = [
        'revenue' => ['title'=>'PENDAPATAN', 'total'=>$report['revenue'] ?? 0],
        'cogs' => ['title'=>'HPP', 'total'=>$report['cogs'] ?? 0],
        'expense' => ['title'=>'BIAYA', 'total'=>$report['expense'] ?? 0],
    ];
...
<div class="card shadow-sm mb-4">
    ...
    <div class="card-body">
        <div class="text-center mb-4"><h5 class="mb-1">{{ $entity->name ?? 'Entitas Utama' }}</h5><div class="fw-semibold">LAPORAN LABA RUGI</div><small class="text-secondary">Periode {{ request('start_date', now()->startOfMonth()->toDateString()) }} s/d {{ request('end_date', now()->endOfMonth()->toDateString()) }}</small></div>
        <div class="table-responsive"><table class="table table-hover align-middle"><thead class="table-light"><tr><th>Kode</th><th>Nama Akun</th><th class="text-end">Jumlah (Rp)</th></tr></thead><tbody>
        @foreach($groups as $type=>$group)
            <tr class="table-light fw-semibold"><td colspan="2">{{ $group['title'] }}</td><td></td></tr>
            @foreach($allCoa->where('type',$type) as $a)
                @php $level=(int)$a['level']; $line=$linesByCode->get($a['code']); $amount=$line['amount']??0; @endphp
                <tr><td style="padding-left:{{ 12 + max(0,$level-1)*20 }}px">{{ $a['code'] }}</td><td class="{{ $level<3?'fw-semibold':'' }}">{{ $a['name'] }}</td><td class="text-end">{{ $level===3?$money($amount):'' }}</td></tr>
            @endforeach
            <tr class="fw-semibold border-top"><td colspan="2">TOTAL {{ $group['title'] }}</td><td class="text-end">{{ $money($group['total']) }}</td></tr>
            @if($type==='cogs')<tr class="fw-semibold"><td colspan="2">LABA KOTOR</td><td class="text-end">{{ $money($report['gross_profit'] ?? 0) }}</td></tr>@endif
        @endforeach
...
@elseif(in_array($module,['purchases','receipts','payables','movements','opname','bom','production','production-results','material-usage','production-cost','fleet','deliveries','operations','fleet-costs','journals'],true))
