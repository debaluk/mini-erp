...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex gap-2 no-print">
        <a href="{{ route('laporan.penjualan.export', request()->query()) }}" class="btn btn-outline-success">📊 Export Excel</a><button type="button" class="btn btn-outline-secondary" onclick="window.print()">🖨 Cetak Penjualan</button>
    </div>
</div>
...
<div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead class="table-light"><tr><th>No. Penjualan</th><th>Tanggal</th><th>Customer</th><th>Unit</th><th>Cara Bayar</th><th>Jatuh Tempo</th><th class="text-end">Subtotal</th><th class="text-end">Diskon</th><th class="text-end">Total</th><th>Status</th><th class="text-end no-print">Aksi</th></tr></thead><tbody>@forelse($rows as $r)<tr><td class="fw-semibold">{{ $r->invoice_no }}</td><td>{{ \Carbon\Carbon::parse($r->sale_date)->format('d/m/Y') }}</td><td>{{ $r->customer_name ?? 'Umum' }}</td><td>{{ $r->unit_name ?? '-' }}</td><td>-</td><td>{{ $r->payment_methods ?? '-' }}</td><td class="text-end">Rp {{ number_format((float)$r->subtotal,0,',','.') }}</td><td class="text-end">Rp {{ number_format((float)$r->discount,0,',','.') }}</td><td class="text-end">Rp {{ number_format((float)$r->total,0,',','.') }}</td><td><span class="badge text-bg-secondary">{{ $r->status }}</span></td><td class="text-end no-print"><button type="button" class="btn btn-sm btn-outline-secondary" onclick="alert('Detail akan diaktifkan setelah logic Penjualan selesai.')">Lihat</button></td></tr>@empty<tr><td colspan="11" class="text-center text-secondary py-4">Belum ada transaksi penjualan.</td></tr>@endforelse</tbody></table></div>
