...
class ErpController extends Controller
{
    ...
    public function supplierMaster(Request $request)
    {
        ...
        if ($request->has('draw')) {
            ...
            $filtered = $query->count();
            $length = max(1, (int) $request->input('length', 15));
            $rows = $query->orderByDesc('id')->offset(max(0, (int) $request->input('start', 0)))->limit($length)
    ...
    public function supplierStore(Request $request)
    {
        ...
        do {$lastNumber++;$code='SUP-'.str_pad((string)$lastNumber,5,'0',STR_PAD_LEFT);}
        ...
        DB::table('suppliers')->insert([
            ...
            'category'=>isset($data['category'])?trim($data['category']):null,'phone'=>isset($data['phone'])?trim($data['phone']):null,
            'whatsapp'=>isset($data['whatsapp'])?trim($data['whatsapp']):null,'email'=>isset($data['email'])?trim($data['email']):null,
            'website'=>isset($data['website'])?trim($data['website']):null,'address'=>isset($data['address'])?trim($data['address']):null,
            'country'=>isset($data['country'])?trim($data['country']):null,'credit_limit'=>0,'is_active'=>(int)$data['is_active'],
    ...
    public function supplierUpdate(Request $request,int $id)
    {
        ...
        $data=$request->validate([
            'name'=>['required','string','max:255'],'category'=>['nullable','string','max:100'],'phone'=>['nullable','string','max:100'],
            'whatsapp'=>['nullable','string','max:100'],'email'=>['nullable','email','max:255'],'website'=>['nullable','string','max:255'],
            'address'=>['nullable','string'],'country'=>['nullable','string','max:100'],'is_active'=>['required','boolean'],
        ]);
