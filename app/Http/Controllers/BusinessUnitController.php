...
class BusinessUnitController extends Controller
{
    ...
    public function edit(Request $request, int $id)
    {
        ...
        return response()->json([
            ...
            'business_type' => $unit->business_type, 'hpp_method' => $unit->hpp_method,
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate([
            'code' => ['required','string','max:50',Rule::unique('business_units','code')->where(fn($q)=>$q->where('entity_id',$entityId))],
        ...
        BusinessUnit::create([
            'entity_id'=>$entityId,'code'=>strtoupper(trim($data['code'])),'name'=>trim($data['name']),
            'business_type'=>$data['business_type'],'hpp_method'=>$data['hpp_method'],
            'is_active'=>$request->boolean('is_active',true),
        ]);
        return back()->with('toast',['type'=>'success','message'=>'Unit bisnis berhasil ditambahkan.','target'=>'setup-unit']);
    }
    ...
    public function update(Request $request, int $id)
    {
        ...
        $data = $request->validate([
            'code' => ['required','string','max:50',Rule::unique('business_units','code')->ignore($unit->id)->where(fn($q)=>$q->where('entity_id',$entityId))],
            'name' => ['required','string','max:150'],
            'business_type' => ['required',Rule::in(['retail','production','service'])],
            'hpp_method' => ['required',Rule::in(['perpetual','periodic','direct_cost'])],
            'is_active' => ['nullable','boolean'],
        ...
        return back()->with('toast',['type'=>'success','message'=>'Unit bisnis berhasil diperbarui.','target'=>'setup-unit']);
    }
    ...
    public function destroy(Request $request, int $id)
    {
        $unit = BusinessUnit::where('entity_id',$this->entityId($request))->findOrFail($id);
        $unit->delete();
        return back()->with('toast',['type'=>'success','message'=>'Unit bisnis berhasil dihapus.','target'=>'setup-unit']);
    }
}
